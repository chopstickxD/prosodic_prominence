...
Input: dataSet - a dataSet which is generated by makeLabels, makeMfcc or read_audio
       ...
       type - 0 for labels/startEndTimes, 1 for feature data, 2 for audio data
'''
...
'''
get the better Labels and match the old ones
Output -  [['name1', [label1]], ['name2', [label2]], ...]
'''
def match2betterLabels(pathName, originalLabels):
    betterLabels = makeLabels(pathName)
    newLabels = []
    for origLabel in originalLabels:
        name = origLabel[0]
        newLabel = getElement(betterLabels, name, 0)
        if len(newLabel) != 0:
            origLabel[1] = newLabel[1]
            newLabel = origLabel
        newLabel = origLabel
...
'''
'''
def makeInputSeq(pathData, pathTimes, pathLabel, maxSentenceStep, maxWordStep, sampleRate, pathBetterLabel=None,
                 useBetter=0):
    ...
    if useBetter == 1:
        betterLabels = match2betterLabels(pathBetterLabel, labelSet)
        seq2label = matchSeq2Label(seqSet, betterLabels)
    else:
    ...
    samplingMethod = 1  # 0 Oversampling, 1 Undersampling, 3 UnderAndOver
    ...
    for arg in word2label:
        ...
        if arg[1] == 1 or arg[1] == 2:
            promLabels.append([0, 1]) #labels prominent[0,1]
            promExamples.append(arg[0])
        else:
            nonpromLabels.append([1, 0]) #notProminent[1,0]
    ...
    while len(nonpromExamples) > len(promExamples):
        ...
        if samplingMethod == 0:
            #oversampling
            randnum = np.random.random_integers(0, len(promExamples)-1)
            promExamples.append(promExamples[randnum])
            promLabels.append([1, 0])


        elif samplingMethod == 1:
            #undersampling
            randnum = np.random.random_integers(0, len(nonpromExamples) - 1)
            del nonpromExamples[randnum]
            del nonpromLabels[randnum]

        else:
            #Under- and Oversampling
            lenNonPromNew = int(len(nonpromExamples)*0.1)
            while len(nonpromExamples) > lenNonPromNew:
                randnum = np.random.random_integers(0, len(nonpromExamples) - 1)
                del nonpromExamples[randnum]
                del nonpromLabels[randnum]
            while lenNonPromNew > len(promExamples):
                randnum = np.random.random_integers(0, len(promExamples) - 1)
                promExamples.append(promExamples[randnum])
                promLabels.append([1, 0])
        ...
        '''
        #SMOTE
        examples = examples.reshape(len(examples), 29*13)
        adsn = ADASYN(k=7, imb_threshold=0.4, ratio=0.65)
        examples, labels = adsn.fit_transform(examples, labels)  # your imbalanced dataset is in X,y
        examples = examples.reshape(len(examples), 29, 13)

        labels_real = []
        for i in range(len(examples)):
            if labels[i] == 1:
                labels_real.append([0, 1])
            else:
                labels_real.append([1, 0])

        '''
    print("DataSet with nonProm datas: ", len(nonpromExamples), " and prom datas: ", len(promExamples))
...
pathTimes = '/home/huynh-tan/Dokumente/Bachelor_Thesis/Labels/StartEndTime_as_csv/startEnd_time_GEN.csv'
pathData = '/home/huynh-tan/Dokumente/Bachelor_Thesis/audio_data/GEN/*16k.wav'
pathLabel = '/home/huynh-tan/Dokumente/Bachelor_Thesis/Labels/Labels_as_csv/labels_GEN.csv'
pathBetterLabel = '/home/huynh-tan/Dokumente/Bachelor_Thesis/Labels/Bette_Labels_as_csv/labels_speaker_GEN.csv'
