...
for n in range(1, 128):
     ...
     for i in range(len(bin)-1, -1, -1):
          zeros[i] = bin[i]
     binaryNumbers[i] = zeros[::-1]
     #print(binaryNumbers[i])


labels = np.zeros((128, 128))
for i in range(0, labels.shape[0]):
          labels[i, labels.shape[1]-1-i]=1
#labels = np.zeros(128)
...
def fill_feed_dict(images_pl, labels_pl):
  ...
  images_feed, labels_feed = binaryNumbers, labels #data_set.next_batch(FLAGS.batch_size,
                              #                   FLAGS.fake_data)
...
def run_training():
  ...
  with tf.Graph().as_default():
    ...
    logits = tf_DNN_model.inference(images_placeholder,
                                    FLAGS.hidden1,
                                    FLAGS.hidden2,
                                    FLAGS.hidden3,
                                    FLAGS.hidden4,
    ...
    loss = tf_DNN_model.loss(logits, labels_placeholder)

    # Add to the Graph the Ops that calculate and apply gradients.
    train_op = tf_DNN_model.training(loss, FLAGS.learning_rate)

    # Add the Op to compare the logits to the labels during evaluation.
    eval_correct = tf_DNN_model.evaluation(logits, labels_placeholder)
